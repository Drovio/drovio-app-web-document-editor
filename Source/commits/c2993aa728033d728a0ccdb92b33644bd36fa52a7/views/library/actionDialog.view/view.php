...
// Build content
$appContent->build("", "actionDialogContainer", TRUE);
$sidebar = HTML::select(".actionDialogContainer .sidebar")->item(0);
...
$title = appLiteral::get("dialog", "lbl_rfolders");
$item = $sMenu->insertListItem("wd_rfolders", $title);
$sMenu->addNavigation($item, $ref = "wDoc_rfolders", $targetcontainer, $targetgroup, $navgroup, $display);
...
$ref_element = HTML::select("#wDoc_rfolders")->item(0);
$sMenu->addNavigationSelector($ref_element, $targetgroup);
...
$form = new simpleForm();
$folderForm = $form->build()->engageApp($appID, "library/createFolder")->get();
...
$form = new simpleForm();
$folderForm = $form->build()->engageApp($appID, "library/createDocument")->get();
DOM::append($folderFormContainer, $folderForm);
...
// Create form
$folderFormContainer = HTML::select(".dlgContainer.rfolders .formContainer")->item(0);
$form = new simpleForm();
$folderForm = $form->build()->engageApp($appID, "library/removeFolder")->get();
DOM::append($folderFormContainer, $folderForm);
