...
if (engine::isPost())
{
	...
	if ($empty)
	{
		...
		// Header
		$header = appLiteral::get("dialog", "lbl_docName");
	...
	$wDoc = new wDoc($parent);
	$status = $wDoc->create($_POST['name']);
	if (!$status)
		return $errFormNtf->getReport();
...
$title = appLiteral::get("dialog", "lbl_createNewDocument");
$wFrame->build($title)->engageApp("library/createDocument");
